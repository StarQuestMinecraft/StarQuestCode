...
public class HarvesterTask extends BukkitRunnable
{
	...
	/**
	 * This assumes that the MovingHarvester was already registered to the
	 * SQTechFarm hashmap.
	 */
	public HarvesterTask(SQTechFarm plugin, MovingHarvester movingHarvester, Player player, Machine machine,
			Harvester harvester)
	{
	...
	public void run()
	{
		...
		if ((rowPosition % 2 != 0 && headPosition == 1)
				|| (rowPosition % 2 == 0 && headPosition == farmHeadSupport.size())) // detect if head is at end
			headAtEnd = true;
		...
		} else
		{
			...
			if (farmableBlock.isEmpty()) // its air
			{
				...
				{
					//TODO implement other crop placing
					if(chest.getInventory().contains(Material.SEEDS))
					{
						farmableBlock.setType(Material.CROPS, false);
						chest.getInventory().removeItem(new ItemStack(Material.SEEDS, 1));
					}
				}
				else if (soilBlock.getType() == Material.SOUL_SAND) // for nether warts
				{
					...
					new BukkitRunnable()
					{
						...
						public void run()
						{
							...
							if (farmableBlock.getType() == Material.NETHER_WARTS
								...
								drops.add(new ItemStack(Material.NETHER_STALK, getDropAmount(Farmables.NETHER_WARTS)));
							else if (farmableBlock.getType() == Material.CROPS
									&& farmableBlock.getData() == Farmables.CROPS.getmaxAge())
								drops.add(new ItemStack(Material.SEEDS, getDropAmount(Farmables.CROPS)));
			...
			if (headAtEnd == true) // when the end rod reaches the end of the iron supports
			{
				if (this.cancelled == true || !plugin.isActive(machine))
					return;
	...
	public void cancel()
	{
		...
		catch (IllegalStateException e)
		{
			Bukkit.getLogger().log(Level.INFO,
					"[SQTechFarm] Someone attempted to turn a harvester off but it was already off.");
		}
		this.cancelled = true;
	}
	...
	public void moveHeadToStart(final MovingHarvester mh, int headPosition)
	{
		...
		final Block head = this.guiBlock.getRelative(BlockFace.UP, 2)
				.getRelative(com.starquestminecraft.sqtechbase.util.DirectionUtils.getLeft(forward), headPosition);
		final List<Block> headSupports = this.harvesterObj
				.detectFarmHeadSupports(this.guiBlock.getRelative(BlockFace.UP), forward);
