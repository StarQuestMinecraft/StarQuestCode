...
public class LargeTankGUI extends GUI{
	
	
	
	public LargeTankGUI(Player player, int id) {
		super(player, id);
	}
	...
	public void open() {
		...
		Inventory gui = Bukkit.createInventory(owner, 27, ChatColor.BLUE + "SQTech - Large Tank");
		...
		if (firstOpen) {
			LargeTankTypeGUI tankTypeGUI = new LargeTankTypeGUI(machine, owner);
			tankTypeGUI.open();
			return;
		}
	...
	public void click(final InventoryClickEvent event) {
		...
		if (event.getClickedInventory().getTitle().startsWith(ChatColor.BLUE + "SQTech")) {
			...
			if (event.getClickedInventory().getTitle().equals(ChatColor.BLUE + "SQTech - Large Tank")) {
