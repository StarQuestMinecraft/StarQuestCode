...
import com.starquestminecraft.sqtechenergy.gui.RedstoneGeneratorGUI;

import net.md_5.bungee.api.ChatColor;

public class RedstoneGenerator extends MachineType {

	public RedstoneGenerator() {

		super(SQTechEnergy.config.getInt("redstone generator.max energy"));
	...
	public boolean detectStructure(GUIBlock guiBlock) {
		...
		Block furnace = block.getRelative(BlockFace.UP);

		Block slab = furnace.getRelative(BlockFace.UP);
		...
		for (BlockFace f : faces) {
			...
			if (block.getType() == Material.LAPIS_BLOCK) {
				if (block.getRelative(right).getType() == Material.STAINED_GLASS_PANE) {
					if (block.getRelative(left).getType() == Material.STAINED_GLASS_PANE) {
						if (block.getRelative(f).getType() == Material.REDSTONE_BLOCK) {
							if (block.getRelative(f).getRelative(right).getType() == Material.STAINED_CLAY) {
								if (block.getRelative(f).getRelative(left).getType() == Material.STAINED_CLAY) {
									...
									if (furnace.getType() == Material.FURNACE || furnace.getType() == Material.BURNING_FURNACE) {
										if (furnace.getRelative(right).getType() == Material.STAINED_GLASS_PANE) {
											if (furnace.getRelative(left).getType() == Material.STAINED_GLASS_PANE) {
												if (furnace.getRelative(f).getType() == Material.REDSTONE_BLOCK) {
													if (furnace.getRelative(f).getRelative(right).getType() == Material.STAINED_CLAY) {
														if (furnace.getRelative(f).getRelative(left).getType() == Material.STAINED_CLAY) {
	...
	public GUI getGUI(Player player, int id) {
		...
		return new RedstoneGeneratorGUI(player, id);
		...
		for (Player player : SQTechBase.currentGui.keySet()) {
			...
			if (SQTechBase.currentGui.get(player).id == machine.getGUIBlock().id) {
				...
				if (player.getOpenInventory() != null) {
					...
					if (player.getOpenInventory().getTitle().equals(ChatColor.BLUE + "SQTech - Redstone Generator")) {
						...
						if (machine.data.get("fuel") != null) {
							...
							if (machine.data.get("fuel") instanceof HashMap<?, ?>) {
								...
								if (fuels.get(0) != null) {
									...
									for (String fuel : SQTechEnergy.config.getConfigurationSection("redstone generator.fuel").getKeys(false)) {
