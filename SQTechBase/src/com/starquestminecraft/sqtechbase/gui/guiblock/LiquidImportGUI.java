...
public class LiquidImportGUI extends GUI {
	
	public LiquidImportGUI(Player player, int id) {
	...
	public void open() {
		...
		Inventory gui = Bukkit.createInventory(owner, 27, ChatColor.BLUE + "SQTech - Liquid Imports");
		...
		for (int i = 0; i < fluids.size(); i ++) {
			...
			gui.setItem(i + 9, Fluid.getOption(machine.liquidImports.contains(fluids.get(i))));
			...
			if (event.getClickedInventory().getTitle().startsWith(ChatColor.BLUE + "SQTech")) {
				...
				if (event.getSlot() > 8) {
					...
					if (fluids.size() + 8 >= event.getSlot()) {
						...
						if (machine.liquidImports.contains(fluids.get(event.getSlot() - 9))) {
							
							machine.liquidImports.remove(fluids.get(event.getSlot() - 9));
							
						} else {
							
							machine.liquidImports.add(fluids.get(event.getSlot() - 9));
							
						}
						
						event.getClickedInventory().setItem(event.getSlot(), Fluid.getOption(machine.liquidImports.contains(fluids.get(event.getSlot() - 9))));
