...
public class CC3Wrapper {
	...
	public static double getBalance(String name, CC3Currency currency) {
		...
		return SQRankup.craftconomy.getAccountManager().getAccount(name).getBalance("default", currency.getName());
	}

	public static double deposit(double amount, String name, CC3Currency currency, Cause cause, String causeReason) {

		return SQRankup.craftconomy.getAccountManager().getAccount(name).deposit(amount, "default", currency.getName(), cause, causeReason);
	}

	public static double withdraw(double amount, String name, CC3Currency currency, Cause cause, String causeReason) {

		return SQRankup.craftconomy.getAccountManager().getAccount(name).withdraw(amount, "default", currency.getName(), cause, causeReason);
	}

	public static boolean hasEnough(double amount, String name, CC3Currency currency) {

		return SQRankup.craftconomy.getAccountManager().getAccount(name).hasEnough(amount, "default", currency.getName());
	}
